"""Encrypted secret storage for veeam-aiops.

Secrets (Veeam login passwords) are NEVER written to disk in plaintext. They
live in ``~/.veeam-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``VEEAM_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
MASTER_PASSWORD_ENV = "VEEAM_AIOPS_MASTER_PASSWORD"
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``VEEAM_<TARGET>_PASSWORD``; ``prefix='VEEAM_'`` and
